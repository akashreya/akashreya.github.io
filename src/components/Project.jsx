        ...
        <AnimatePresence>
          {showModal && (
            ...
            >
              <motion.div
                ref={modalRef}
                className="bg-white dark:bg-slate-800 p-6 rounded-[20px] max-w-[600px] w-full shadow-lg"
                initial={{ scale: 0.95, opacity: 0 }}
                animate={{ scale: 1, opacity: 1 }}
                exit={{ scale: 0.95, opacity: 0 }}
                transition={{ duration: 0.3 }}
              >
                <h3 className="text-[16px] font-bold mb-4">{project.title}</h3>
                <p className="mb-6">{project.description}</p>
                <button
                  onClick={() => setShowModal(false)}
                  className="cursor-pointer px-4 py-2 bg-blue-600 text-white rounded-[15px] hover:bg-blue-900"
                >
                  Close
                </button>
              </motion.div>
