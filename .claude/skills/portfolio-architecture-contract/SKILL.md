...
| 12 | Personal-mode Enterprise strip ignored API data (hardcoded "13y…" while recruiter alone rendered `site.enterprise`) | `Enterprise.jsx` | Hardcoded fact drifted from API content on the same page (13y vs the ticker's 15 years). | **FIXED 2026-07-12** — strip copy now comes from `sections.enterprise.collapsed` (site payload, 15y), and personal mode renders `site.enterprise` as mini cards (client/role/period) under the "9 to 5" head. Section headings generally moved to the API `sections` node the same day (SectionHead component, `resolveSections` in client.js). |
