...
| 9 | Frontend never fetches `/api/sidequests` or `/api/ticker` — `SideQuests` and `HeroTicker` content is hardcoded (see §4); the endpoints are live on the backend but unseeded (`200 []`) | `SideQuests.jsx:13-21`, `HeroTicker.jsx:1-12` | Flag-flip alone ships hardcoded content; API wiring is unbuilt campaign work. Detail: `portfolio-personal-mode-campaign`. | **OPEN** — requires backend seeding + owner content decisions; out of scope for a code-only fix pass. |
